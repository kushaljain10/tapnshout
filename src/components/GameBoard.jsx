...
export default function GameBoard() {
  ...
  // Tap handlers
  const handleTap = useCallback((clientY) => {
    if (winner) return
  ...
  return (
    <div ref={containerRef} className="game-container" onMouseDown={onMouseDown} onTouchStart={onTouchStart}>
      <div className="half top" />
      <div className="half bottom" />
      <div className="center-line" />

      <svg className="svg-stage" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        <defs>
          <filter id="ballGlow" x="-50%" y="-50%" width="200%" height="200%">
            <feDropShadow dx="0" dy="2" stdDeviation="2" floodColor="#ffffff" floodOpacity="0.35" />
          </filter>
        </defs>
        {/* Path */}
        <path ref={pathRef} d={pathD} stroke="#fff" strokeWidth="3" fill="none" opacity="0.9" />
        {/* Trail */}
        {trailEls}
        {/* Ball (slightly larger) */}
        <circle ref={ballRef} className="ball" r="14" fill="var(--ball)" filter="url(#ballGlow)" />
      </svg>
